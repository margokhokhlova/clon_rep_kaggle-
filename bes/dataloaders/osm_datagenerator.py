...
from dataloaders.img_helper import  make_multi_class_image
import numpy as np
# Ignore warnings
import warnings
...
class DataGeneratorOSM(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            if self.label_mask == 'road':
                ys = np.expand_dims(1.0-(ys[:, :, 0]/255.0), axis=3)  # take only the red channel of the image
            if self.label_mask is None:
                ys = make_multi_class_image(ys, ignore_channel=1) # ignore the G channel - it is not used

            X[i,] = Xs
            # Store class
            y[i,] = ys
            imsave(label_name[:-4]+'binary.png',np.squeeze(ys))
